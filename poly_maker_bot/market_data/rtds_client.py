...
class RealTimeDataClient:
    ...
    def __init__(
        ...
        # Application-level heartbeat (Polymarket expects text "PING")
        self._heartbeat_timer: Optional[threading.Timer] = None

        # Reconnection management (fast initial retries, then exponential backoff)
        self._reconnect_attempts = 0
        self._initial_reconnect_delay = 0.05  # Start at 50ms for fast recovery
        self._max_reconnect_delay = 30  # Cap at 30s (not 5 minutes) for trading
    ...
    def connect(self) -> 'RealTimeDataClient':
        ...
        # Run in separate thread (no built-in ping - Polymarket uses application-level heartbeat)
        self._ws_thread = threading.Thread(
            target=self.ws.run_forever,
            kwargs={"ping_interval": 0, "ping_timeout": None},  # Disable built-in pings
            daemon=True
        )
    ...
    def _on_message(self, _ws, raw_message: str):
        ...
        try:
            ...
            # Log ALL non-JSON messages to debug pong
            if not raw_message.strip().startswith("{"):
                logger.info(f"Non-JSON message received: '{raw_message}' (type: {type(raw_message).__name__}, len: {len(raw_message)})")

                # Check for pong response (application-level heartbeat) - case-insensitive
    ...
    def _on_pong(self, _ws, message):
        """WebSocket pong handler (for application-level heartbeat)."""
    ...
    def _schedule_reconnect(self) -> None:
        """Schedule a reconnection with fast initial retries then exponential backoff."""
        # Cancel any existing reconnect timer
        if self._reconnect_timer:
            self._reconnect_timer.cancel()
            self._reconnect_timer = None

        # Fast initial retries: 0.5s, 1s, 2s, 4s, 8s, 16s, 30s max
        # Formula: initial_delay * 2^attempts, capped at max_delay
        delay = min(
            self._initial_reconnect_delay * (2 ** self._reconnect_attempts),
    ...
    def _send_heartbeat(self):
        """Send application-level heartbeat (text 'ping')."""
    ...
    def _schedule_heartbeat(self):
        ...
        if self._shutdown_requested:
            return
        self._heartbeat_timer = threading.Timer(self.ping_interval, self._send_heartbeat)
